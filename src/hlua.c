...
static int hlua_lua2arg(lua_State *L, int ud, struct arg *arg)
{
	...
	case LUA_TSTRING:
		...
		arg->data.str.size = arg->data.str.len;
		break;
...
static int hlua_lua2smp(lua_State *L, int ud, struct sample *smp)
{
	...
	case LUA_TSTRING:
		...
		/* We don't know the actual size of the underlying allocation, so be conservative. */
		smp->data.u.str.size = smp->data.u.str.len;
		break;
...
static int hlua_socket_write_yield(struct lua_State *L,int status, lua_KContext ctx)
{
	...
	if (len < send_len)
		send_len = len;
	len = bi_putblk(&socket->s->req, buf, send_len);
...
static void hlua_applet_http_fct(struct appctx *ctx)
{
	...
	if (!HLUA_IS_RUNNING(hlua) &&
		...
		bo_skip(si_oc(si), strm->txn->req.eoh + strm->txn->req.eol);
	}
